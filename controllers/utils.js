...
const base64ToJimpInstance = (base64) => {
	...
	return Jimp.read(imageBuffer).then((image) =>
		image.resize(Jimp.AUTO, 450).quality(75)
	);
};

const uploadImagesToDisk = (images, folder) => {
	return Promise.all(
		images.map(({ buffer, imageId }) =>
			base64ToJimpInstance(buffer).then((instance) => ({ instance, imageId }))
		)
	).then((jimpInstances) => {
		jimpInstances.forEach(({ instance, imageId }) =>
			instance.writeAsync(`images/${folder}/${imageId}.jpeg`)
		);
	});
};
